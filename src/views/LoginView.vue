...
export default {
  ...
  getLogin(){
    ...
    //Si succès, on stocke les données de l'utilisateur dans le store, puis on redirige vers la page d'accueil
    .then(
      (data)=>{
        //Enregistrement dans le store, dans la varaible utilisateur
        this.$store.state.utilisateur = data;
        console.log(data);
        this.successMessage = 'Bien vue chackal !';
      ...
      .catch(
        ()=>{
          //this.isVisible=true
          this.errorMessage = 'Identifiant ou mot de passe mauvais bouffon !!';
